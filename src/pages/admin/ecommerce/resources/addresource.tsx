...
const AddResources = () => {
    ...
    const [resourcesubimage, setresourcesubimage] = useState('')
    const {
        register,
        handleSubmit,
        setValue,
    ...
    const handleChange = (e: any) => {
        ...
        if (e.target.name === "resourceimg" && file) {
            setValue("resourceimg", file);
            reader.onload = (e: any) => {
                setresourceimage(e.target.result);
            };
            reader.readAsDataURL(file);
        } else if (e.target.name === "resourcesubimg" && multiplefile) {
            setValue("resourcesubimg", file);
            reader.onload = (e: any) => {
                setresourcesubimage(e.target.result);
            };
            reader.readAsDataURL(file);
        };
    };
    const resetresourceimage = () => {
        setresourceimage("");
    }
    const resetresourcesubimage = () => {
        setresourcesubimage("");
    }
                                    ...
                                    {resourceimage ?
                                        <div className="flex flex-col items-center justify-center w-full h-32  border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 mt-1.5">
                                            <div className="img_wrp ">
                                                <img className="object-cover h-24 w-24" src={resourceimage} />
                                                <div onClick={resetresourceimage} className="absolute top-0 right-0 text-red-600	 "><RiCloseCircleFill /></div>
                                            </div></div>
                                        : (
                                            <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-32  border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 mt-1.5">
                                                ...
                                                <input id="dropzone-file" {...register("resourceimg")} onChange={handleChange} type="file" className="hidden" />
                                            </label>)}
                                </div>
                                <div className="md:w-1/2 px-3">
                                    <label className="block text-sm">
                                    ...
                                    {resourcesubimage ?
                                        <div className="flex flex-col items-center justify-center w-full h-32  border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 mt-1.5">
                                            <div className="img_wrp ">
                                                <img className="object-cover h-24 w-24" src={resourcesubimage} />
                                                <div onClick={resetresourcesubimage} className="absolute top-0 right-0 text-red-600	 "><RiCloseCircleFill /></div>
                                            </div></div>
                                        : (
                                            <label htmlFor="dropzone-file1" className="flex flex-col items-center justify-center w-full h-32  border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 mt-1.5">
                                                ...
                                                <input id="dropzone-file1"  {...register("resourcesubimg")} onChange={handleChange} type="file" multiple className="hidden" />
                                            </label>
                                        )}
                                </div>
                            </div>
