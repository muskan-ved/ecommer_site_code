  ...
  return (
    <section className="flex gap-6 ">
      <div
        ...
        onMouseOver={() => {
          setOpen(true);
          selectedSubnavTitle !== "" && setSubMenuOpen(true);
        }}
        onMouseOut={() => setOpen(false)}
      >
        <div
          className="py-3 flex justify-end rounded-md cursor-pointer hover:bg-gray-200"
          onClick={() => setOpen(!open)}
        >
          {open ? (
            <div className="flex items-center space-x-3 lg:pr-18 pr-6 ">
              <svg
            ...
            <NavLink to={`${Menu?.path}`} key={index}>
              <li
                className={` relative flex flex-wrap rounded-md p-2 cursor-pointer hover:bg-gray-200 text-black text-sm items-center gap-x-4 
                                mt-0.5 ${
                                  subMenuText === Menu.name && "bg-gray-200"
                                }`}
                          ...
                          {Menu.subMenus.map(
                            (subMenuItem: any, idx: number): any => (
                              <NavLink
                                ...
                                key={idx}
                                onClick={() =>
                                  setSelectedSubnavTitle(subMenuItem.name)
                                }
                              >
                                <li
                                  className={`flex rounded-md p-2  cursor-pointer hover:bg-gray-300 text-black text-sm items-center gap-x-4 w-56 ${
                                    selectedSubnavTitle === subMenuItem.name &&
                                    "bg-gray-300"
                                  }`}
                                >
                                  <div>
